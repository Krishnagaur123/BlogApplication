...
class ApiService {
  ...
  public async register(user: RegisterRequest): Promise<string> {
    const response: AxiosResponse<string> = await this.api.post('/auth/register', user);
    return response.data; // "User registered successfully"
  }

  public async getUserProfile(): Promise<AuthUser> {
    const response: AxiosResponse<AuthUser> = await this.api.get('/users/me');
    return response.data;
  }
